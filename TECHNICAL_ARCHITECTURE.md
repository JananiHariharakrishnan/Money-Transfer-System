...
| **Dashboard** | Home page | Account balance, action buttons, 4x2 grid |
...
| **Profile** | Account info | Read-only account details with status |
...
|--------|----------|------|---------|
| GET | `/auth` | Basic | Validate credentials |
| GET | `/api/v1/accounts/{id}` | Basic | Fetch account details |
| GET | `/api/v1/accounts/{id}/transactions` | Basic | Fetch transaction history |
...
- **Stateless**: Each request must include credentials
